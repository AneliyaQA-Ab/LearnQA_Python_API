...
class TestUserGet(BaseCase):
    def test_get_user_details_not_auth(self):
         response =  requests.get("https://playground.learnqa.ru/api/user/2") # захардкодили id  = 2
    ...
    def test_get_user_details_auth_as_not_same_user(self):
        ...
        response2 = requests.get(
            f"https://playground.learnqa.ru/api/user/1", #захардкодили, для логина и пароля выше id = 2
            headers={"x-csrf-token":token},
            cookies= {"auth_sid":auth_sid}
        )
